...
let pransu = {name: 'pransu pathak'},
    samiksha = {name: 'samiksha dube'},
    atul = {name:'atul pathak'}
    vedanta = {name: 'vedanta rai'};
...
for (const user of userRoles.keys()) {//iterate over map key()method
  console.log(user.name);
}

for (let role of userRoles.values()) { //iterate over map value()method
  console.log(role);
}
for (let [user, role] of userRoles.entries()) {//entrires() method
  console.log(`${user.name}: ${role}`);
}

userRoles.forEach((role, user) => //in first parameter take role otherwise you get undefined pair
  console.log(`${user.name}: ${role}`));


console.log(userRoles instanceof Map);

userRoles.set(atul, 'trainer');
console.log(userRoles.get(pransu));//get() method
console.log(userRoles.get(atul));

console.log(userRoles.has(vedanta));//has() method

console.log(userRoles.size);//size property
var keys = [...userRoles.keys()];// using spreed operator
console.log(keys);
var roles = [...userRoles.values()];
console.log(roles);
console.log(userRoles.delete(atul));
console.log(userRoles.size);
console.log(userRoles.clear());
console.log(userRoles.size);

//Set()
let chars = new Set(['a','b','c','d','e']);
console.log(chars);
console.log(chars.size);
chars.add('f');
console.log(chars.size);
console.log(chars.has('z'));
console.log(chars.clear());
console.log(chars.size);


let roleName = new Set();
roleName.add('user')
     .add('developer')
     .add('trainer');
...
for (let [key, value] of roleName.entries()) {
  console.log(key === value);
}

//Proxy()
// here user– is an object to wrap.
...
const user = {  //user object define 
  firstName: 'pransu',
  lastName: 'Pathak',
  email: 'pranpathak01@gmail.com',
}
const hander ={  //handler object define
  get(target,property){
    console.log(`properties ${property} is -`);
    return target[property];
  }

}
const proxyUser = new Proxy(user,hander); // create Proxy object
console.log(proxyUser.firstName);
console.log(proxyUser.lastName);
console.log(proxyUser.email);
user.firstName = 'vendana';  //change in user
console.log(proxyUser.firstName);

proxyUser.lastName = 'dube';//change in proxuUser
